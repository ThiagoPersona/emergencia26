...
```mermaid
flowchart TD
    A[Paciente com indicação de via aérea] --> B{Parado ou agônico?}
    B -->|Sim| C[Algoritmo de PCR: BVM, compressão, DEG/IOT conforme equipe]
    B -->|Não| D{predição de via aérea difícil?}
    D -->|Não| E[RSI preparada]
    D -->|Sim| F{Consigo oxigenar e esperar?}
    F -->|Sim| G[Plano acordado/duplo preparo: especialista, vídeo/fibro, crico pronta]
    F -->|Não| H[Forçado a agir: melhor tentativa + plano de resgate imediato]
    E --> I{IOT confirmou por capnografia?}
    G --> I
    H --> I
    I -->|Sim| J[Pós-IOT: sedação, VM, gasometria, Rx/POCUS, DOPE se piora]
    I -->|Não| K[Reoxigenar e mudar estratégia]
    K --> L{Oxigena com BVM/DEG?}
    L -->|Sim| M[Nova tentativa otimizada ou via aérea definitiva alternativa]
    L -->|Não| N[NINO/CICO: cricotireoidostomia]
```
...
```mermaid
flowchart TD
    A[Paciente em VM com obstrução/DPOC/asma] --> B{Fluxo expiratório volta a zero?}
    B -->|Sim| C[Sem auto-PEEP relevante na curva]
    B -->|Não| D[Auto-PEEP provável]
    D --> E[Reduzir FR]
    E --> F[Aumentar tempo expiratório]
    F --> G[Tratar broncoespasmo e secreção]
    G --> H[Reavaliar PA, curva fluxo-tempo e gasometria]
```
