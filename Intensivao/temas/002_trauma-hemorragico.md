...
```mermaid
flowchart TD
    A[Trauma com sangramento ou choque] --> B[X: sangramento externo exsanguinante?]
    B -->|Sim| C[Compressão/packing/torniquete + registrar horário]
    B -->|Não| D[ABCDE com foco em perfusão]
    C --> D
    D --> E{Instável ou hipoperfundido?}
    E -->|Não| F[Exame secundário + TC conforme mecanismo]
    E -->|Sim| G[eFAST + tórax + pelve + ossos longos]
    G --> H{Fonte provável?}
    H -->|Abdome/eFAST positivo| I[Cirurgia/laparotomia; ativar transfusão maciça]
    H -->|Tórax| J[Drenagem; toracotomia se maciço/persistente]
    H -->|Pelve| K[Binder/lençol + sangue + embolização/tamponamento]
    H -->|Extremidade| L[Torniquete/imobilizar/controle vascular]
    H -->|Indefinida| M[Ressuscitação hemostática + busca ativa com POCUS/cirurgia]
    I --> N[TXA + cálcio + aquecer + coagulopatia/ROTEM]
    J --> N
    K --> N
    L --> N
    M --> N
