...
```mermaid
flowchart TD
    A["Irresponsivo, sem respiração normal"] --> B["Chamar ajuda + desfibrilador + iniciar RCP"]
    B --> C["Monitor: ritmo?"]
    C -->|FV/TV sem pulso| D["Choque"]
    D --> E["RCP 2 min imediatamente"]
    E --> F["Acesso IV/IO + capnografia + causas"]
    F --> C
    C -->|AESP/assistolia| G["RCP 2 min + adrenalina cedo"]
    G --> H["5H/5T + confirmar assistolia se aplicável"]
    H --> C
    C -->|RCE| I["Pós-RCE: O2, CO2, PA, ECG, causa, temperatura"]
    C -->|Sem RCE e critério de interrupção| J["Decisão estruturada; não usar EtCO2 isolado"]
```
...
```mermaid
flowchart TD
    A["RCE"] --> B["ABCDE + confirmar tubo/capnografia"]
    B --> C["SpO2 adequada e normocapnia"]
    C --> D["Tratar hipotensão: volume se indicado + vasoativo"]
    D --> E["ECG 12 derivações"]
    E -->|IAMCST ou instabilidade| F["Hemodinâmica/cateterismo"]
    E -->|Sem IAMCST claro| G["Investigar causa: POCUS, gaso, labs, TC se indicado"]
    F --> H["UTI + controle de temperatura se comatoso"]
    G --> H
    H --> I["Convulsão/EEG, glicemia, prognóstico tardio multimodal"]
```
