...
```mermaid
flowchart TD
    A["Choque ou hipoperfusão"] --> B["RCP/ABCDE se crítico + monitor + acesso + drogas tempo-dependentes"]
    B --> C["POCUS RUSH: Pump, Tank, Pipes"]
    C --> D{"Bomba alterada?"}
    D -->|VE ruim + B-lines/VCI cheia| E["Cardiogênico: vasopressor/inotrópico, SCA/arritmia/IC"]
    D -->|VD grande/septo D| F["Obstrutivo: TEP/cor pulmonale; procurar TVP e contexto"]
...
```mermaid
flowchart TD
    A["Dispneia grave"] --> B["US pulmonar anterior/lateral + coração se instável"]
    B --> C{"Sliding?"}
    C -->|Ausente + A-lines| D["Pneumotórax possível: procurar lung point; tratar se instável"]
    C -->|Presente| E{"Perfil predominante"}
    E -->|B bilateral difuso| F["Edema/congestão: SCAPE/IC/SDRA conforme contexto"]
    E -->|A bilateral| G["Asma/DPOC/TEP/outros; procurar DVT/VD/PLAPS se suspeita"]
    E -->|Consolidação ou B focal| H["Pneumonia/contusão/atelectasia"]
    E -->|Derrame| I["Quantificar, etiologia, toracocentese se indicado"]
```
...
```mermaid
flowchart TD
    A["PCR em RCP"] --> B["Definir Pergunta antes da pausa"]
    B --> C["Probe posicionado durante compressões"]
    C --> D["Pausa <10 s: imagem CASA"]
    D --> E{"Achado tratável?"}
    E -->|Tamponamento| F["Pericardiocentese/cirurgia conforme contexto"]
    E -->|VD muito dilatado + contexto TEP| G["Considerar trombólise e RCP prolongada"]
    E -->|Pneumotórax| H["Descompressão/drenagem"]
    E -->|Atividade cardíaca sem pulso| I["Pseudo-AESP: tratar choque grave/causa"]
    E -->|Sem achado| J["Voltar ao algoritmo ACLS/PALS e 5H/5T"]
```
