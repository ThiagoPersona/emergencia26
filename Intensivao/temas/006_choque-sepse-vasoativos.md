...
flowchart TD
    ...
    C -->|Tamponamento| F["Pericardiocentese/cirurgia"]
    C -->|Anafilaxia| G["Adrenalina IM + via aérea + volume"]
    C -->|Arritmia/SCA| H["Choque/cardioversão/reperfusão conforme caso"]
    C -->|Indefinida| I["Fenotipar: hipovolêmico, distributivo, cardiogênico, obstrutivo"]
    I --> J{"Fluido-responsivo e tolerante?"}
    J -->|Sim| K["Cristaloide/sangue conforme causa + reavaliar"]
...
```mermaid
flowchart TD
    A["Paciente em choque"] --> B{"Qual problema dominante?"}
    B -->|Vasoplegia| C["Noradrenalina"]
    B -->|Nora subindo| D["Adicionar vasopressina 0,03 U/min"]
    B -->|refratário| E["Adrenalina +/- hidrocortisona"]
    B -->|Baixo débito com PAM ok ou nora| F["Dobutamina"]
    B -->|Anafilaxia| G["Adrenalina IM primeiro"]
    B -->|Obstrutivo| H["Tratar obstrução: TEP/tamponamento/pneumo"]
```
