...
```mermaid
flowchart TD
    A[Déficit focal súbito] --> B[ABCDE, glicemia, hora do último visto bem]
    B --> C[NIHSS, PA, anticoagulante, mRS prévio]
    C --> D[TC crânio sem contraste]
    D -->|Hemorragia| E[HIP/HSA: controlar PA, reverter anticoagulante, neurocirurgia]
...
flowchart TD
    ...
    E -->|Não| G[Segunda linha: fenitoína/fosfenitoina, levetiracetam ou valproato]
    G --> H{Persiste?}
    H -->|Sim| I[IOT, sedação anestesica, EEG/UTI, tratar causa]
    C -->|Não| F
```
...
```mermaid
flowchart TD
    A[TCE/HSA/HIP/coma com sinais de HIC] --> B[ABCDE, glicemia, pupilas, Glasgow]
    B --> C[Oxigenar, PA adequada, cabeça 30 graus, pescoço neutro]
    C --> D{Herniação iminente?}
    D -->|Sim| E[Hipertônica ou manitol + hiperventilação temporária]
    D -->|Não| F[Normocapnia, analgesia/sedação, evitar febre]
    E --> G[TC e neurocirurgia imediata]
    F --> G
    G --> H[Reverter anticoagulante/coagulopatia e definir UTI/centro neuro]
```
