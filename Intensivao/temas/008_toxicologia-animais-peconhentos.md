...
```mermaid
flowchart TD
    A[Paciente intoxicado ou exposição suspeita] --> B[ABCDE + EPI + glicemia + ECG + temperatura]
    B --> C{Instável?}
    C -->|Hipoventila| D[Ventilar/O2; considerar naloxona se opioide]
    ...
    G --> I
    H --> I
    I --> J{Descontaminação segura é útil?}
    J -->|Sim| K[Carvão/PEG/lavagem excepcional]
    J -->|Não| L[Suporte e observação]
    K --> M[antídoto específico se indicado]
    L --> M
    M --> N[CIATox, destino: alta observada, enfermaria, UTI ou diálise]
```
...
flowchart TD
    ...
    C -->|Sangramento tardio após lagarta| I[Lonomia: coagulograma/fibrinogênio, soro antilonomico]
    D --> J[Notificar, observar e reavaliar gravidade]
    E --> J
    F --> J
    G --> J
    H --> J
