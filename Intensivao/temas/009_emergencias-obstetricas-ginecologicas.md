...
```mermaid
flowchart TD
    A[Gestante >20 sem ou puérpera com crise, cefaleia, escotomas ou PA grave] --> B[ABCDE, lateralizar, glicemia, acesso, monitor]
    ...
    G --> H
    H --> I[Hemograma, plaquetas, creatinina, TGO/TGP, DHL, proteinúria, coagulograma se sangra]
    I --> J{Déficit focal, coma persistente ou crise recorrente?}
    J -->|Sim| K[TC/angio conforme caso + UTI/neuro]
    J -->|Não| L[Obstetrícia: definir parto após estabilização]
```
...
flowchart TD
    ...
    K -->|Sim| L[DPP: fibrinogênio, sangue, parto conforme mãe/feto]
    K -->|Não| M[Placenta prévia: US, suporte, obstetrícia]
```
