...
```mermaid
flowchart TD
    A[Choque pediátrico] --> B{Hemorragia/trauma?}
    B -->|Sim| C[Controle de fonte + CH 10 mL/kg + aquecer + cirurgia/trauma]
    B -->|Não| D{Diarreia/vômitos/desidratação?}
    D -->|Sim| E[Cristaloide isotônico 20 mL/kg + reavaliar]
    D -->|Não| F{Febre/infecção suspeita?}
    F -->|Sim| G[Sepse: antibiótico precoce + bolus 10-20 mL/kg + vasoativo se refratário]
    F -->|Não| H[Considerar cardiogênico, anafilaxia, tóxico, endócrino/metabólico]
```
