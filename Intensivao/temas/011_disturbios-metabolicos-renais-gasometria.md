...
```mermaid
flowchart TD
    A[Paciente com alteração metabólica grave] --> B[ABCDE + glicemia + ECG + acesso + monitor]
    B --> C{Ameaça imediata?}
    C -->|Hipoglicemia| D[Glicose EV/VO conforme consciência]
    C -->|HiperK/ECG| E[Cálcio EV -> insulina/glicose -> beta2 -> remover K]
    C -->|Hiponatremia com convulsão/coma| F[NaCl 3% em bolus + reavaliar]
    C -->|Crise adrenal| G[Hidrocortisona + SF 0,9% + dextrose se preciso]
    C -->|CAD/EHH/choque| H[Cristaloide + K antes da insulina + gatilho]
    C -->|Nenhuma| I[Interpretar gaso, eletrólitos, renal, tóxico e endócrino]
```
...
flowchart TD
    ...
    B -->|<7,35| C[Acidemia]
    B -->|>7,45| D[Alcalemia]
    C --> E{HCO3 baixo ou PaCO2 alto?}
    D --> F{HCO3 alto ou PaCO2 baixo?}
    E -->|HCO3 baixo| G[Acidose metabólica]
    E -->|PaCO2 alto| H[Acidose respiratória]
    F -->|HCO3 alto| I[Alcalose metabólica]
    F -->|PaCO2 baixo| J[Alcalose respiratória]
    G --> K[Calcular compensação de Winter]
    K --> L[Calcular AG corrigido e delta]
    H --> M[Aguda vs crônica pela resposta do HCO3]
```
...
```mermaid
flowchart TD
    A[IRA/DRC descompensada] --> B[Volume/perfusão + urina + EAS + eletrólitos + POCUS]
    B --> C{Obstrução?}
