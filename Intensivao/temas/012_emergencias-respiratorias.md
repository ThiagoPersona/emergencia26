...
```mermaid
flowchart TD
    A[Dispneia grave] --> B[ABCDE + O2 com alvo + monitor + ECG + acesso]
    B --> C{Falência de via aérea/ventilação ou choque?}
    C -->|Sim| D[BVM/IOT ou VNI se DPOC/EAP e critérios favoraveis]
    C -->|Não| E[Gaso + RX/POCUS + labs dirigidos]
    E --> F{POCUS/clínica sugere?}
    F -->|A com sibilos| G[Asma/DPOC: broncodilatador, corticoide, VNI se hipercapnia]
    F -->|B difuso| H[EAP/SDRA: PEEP, causa, diurético se cardiogênico]
    F -->|Consolidação/derrame| I[Pneumonia/pleural: antibiótico, drenagem se empiema]
    F -->|A' ou lung point| J[Pneumotórax: descompressão/dreno se instável]
    F -->|VD grande/TVP| K[TEP: anticoagular, reperfusão se alto risco]
```
...
flowchart TD
    ...
    F -->|Não| H[IOT e VM obstrutiva]
    D -->|Não| I[Tratamento clínico e observação]
```

```mermaid
flowchart TD
    A[Suspeita de TEP] --> B{instável/choque/PCR?}
    B -->|Sim| C[O2, noradrenalina, UFH se possível, POCUS/eco/TVP]
    C --> D{Alta probabilidade e sem contra maior?}
    D -->|Sim| E[Trombólise/reperfusão]
    D -->|Não| F[Suporte + angioTC se estabilizar + equipe especializada]
...
```mermaid
flowchart TD
    A[SDRA suspeita] --> B[P/F <=300 + opacidades bilaterais + PEEP >=5]
    B --> C[Tratar causa + VM protetora]
    C --> D[VC 4-8 mL/kg PBW; Pplat <30; DP <=15]
    D --> E{P/F <150?}
    E -->|Sim| F[Prona precoce 16h ou mais + PEEP adequada]
    E -->|Não| G[Manter pacote e reavaliar]
    F --> H{Hipoxemia refratária?}
    H -->|Sim| I[NMBA/ECMO conforme recurso e perfil]
    H -->|Não| G
```
