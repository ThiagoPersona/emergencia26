...
```mermaid
flowchart TD
    A[Dor abdominal aguda] --> B[ABCDE + analgesia + acesso + labs + beta-hCG se aplicável]
    B --> C{Choque, peritonite ou sangramento ativo?}
    C -->|Sim| D[Ressuscitar + cirurgia/endoscopia/vascular/IR conforme suspeita]
    C -->|Não| E{Red flag: idoso, cirrose, imunossupressão, lactato, dor desproporcional?}
    E -->|Sim| F[TC/angioTC ou US dirigido com baixo limiar + especialista]
    E -->|Não| G[Imagem dirigida: US biliar/gineco, TC, POCUS, EAS]
    G --> H{Fonte control?}
    F --> H
    H -->|Sim| I[Cirurgia, CPRE, drenagem, endoscopia ou urologia]
    H -->|Não| J[Tratamento clínico + reavaliação seriada]
```
...
flowchart TD
    ...
    E --> F
    F -->|Sim| G[Endoscopia terapêutica, IR/cirurgia/TIPS conforme causa]
    F -->|Não| H[Monitorar, tratar causa, plano de prevenção]
```
...
flowchart TD
    ...
    A --> I{Hematemese/melena?}
    I -->|Sim| J[HDA varicosa até prova: vasoativo + ATB + EDA]
```
...
flowchart TD
    ...
    I -->|Sim| J[Colecistite: ATB + colecistectomia precoce]
```
