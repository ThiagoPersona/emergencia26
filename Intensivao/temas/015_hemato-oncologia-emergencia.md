...
```mermaid
flowchart TD
    A[Paciente oncológico/hematológico no DE] --> B[ABCDE + sinais vitais + hemograma/diferencial + labs]
    B --> C{Febre e ANC <500 ou queda prevista?}
    C -->|Sim| D[Culturas se não atrasar + beta-lactâmico antipseudomonas até 60 min]
    C -->|Não| E{HiperK/hiperP/úrico alto/IRA?}
    E -->|Sim| F[Tratar hiperK + hidratar + rasburicase/allopurinol + nefro/heme]
    E -->|Não| G{Neuro, via aérea, sangramento ou dor coluna?}
    G -->|Sim| H[MSCC/SVCS/leucostase/TTP: especialista e imagem/procedimento urgente]
    G -->|Não| I[Tratamento dirigido + reavaliação seriada]
```
...
flowchart TD
    ...
    F --> G
    G -->|MASCC >=21/CISNE baixo e suporte| H[Observar, dose inicial, considerar VO ambulatorial]
    G -->|Alto risco ou dúvida| I[Internar/UTI conforme gravidade]
```

```mermaid
flowchart TD
    A[Síndrome de lise tumoral] --> B[ECG + monitor + K/P/Ca/úrico/Cr seriados]
    B --> C{HiperK com ECG ou instabilidade?}
    C -->|Sim| D[Cálcio -> insulina/glicose -> beta2 -> remover K]
    C -->|Não| E[Hidratação e controle laboratorial]
    D --> E
    E --> F{Alto risco ou úrico alto/IRA?}
    F -->|Sim| G[Rasburicase se sem contra + heme/nefro]
    F -->|Profilaxia/intermediario| H[Allopurinol + hidratação]
    G --> I{AEIOU/refratário?}
    H --> I
    I -->|Sim| J[Diálise]
    I -->|Não| K[Monitorar e tratar tumor]
```

```mermaid
flowchart TD
    A[Piora durante transfusão] --> B[Parar transfusão + SF 0,9 em equipo novo]
    B --> C[Conferir identificacao + avisar banco de sangue]
    C --> D{fenótipo}
    D -->|Hipotensão/dor lombar/hemoglobinúria| E[Hemolítica: suporte + investigação + UTI]
    D -->|Hipoxemia sem congestão| F[TRALI: suporte ventilatório]
    D -->|Hipertensão/JVP/congestão| G[TACO: O2 + diurético]
    D -->|urticária/broncoespasmo/choque| H[alérgica/anafilaxia: anti-H1 ou adrenalina IM]
    D -->|Febre alta/choque| I[Sepse transfusional: culturas + ATB amplo]
```
