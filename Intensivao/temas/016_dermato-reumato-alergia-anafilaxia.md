...
```mermaid
flowchart TD
    A[Rash, edema ou lesão de pele no DE] --> B[ABCDE: via aérea, broncoespasmo, hipotensão, febre, dor]
    B --> C{Anafilaxia?}
    C -->|Sim| D[Adrenalina IM + O2 + fluidos + repetir 5-15 min]
    C -->|Não| E{Angioedema de via aérea?}
...
```mermaid
flowchart TD
    A[Anafilaxia suspeita] --> B[Adrenalina IM vasto lateral]
...
```mermaid
flowchart TD
    A[Monoartrite aguda] --> B{Sepse/choque?}
    B -->|Sim| C[Hemoculturas se possível + antibiótico imediato + artrocentese assim que viável]
    B -->|Não| D[Artrocentese antes do antibiótico]
    D --> E[Gram, cultura, celularidade, cristais]
    E --> F{Pus/Gram/cultura ou alta suspeita?}
    F -->|Sim| G[ATB EV + drenagem + ortopedia/reumato]
    F -->|Cristais e baixa suspeita infecção| H[AINE/colchicina/corticoide conforme perfil]
```
